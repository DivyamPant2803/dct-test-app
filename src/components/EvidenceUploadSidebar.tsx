...
const LegalRequirementText = styled.div`
  background: white;
  padding: 1rem;
  border-radius: 4px;
  border: 1px solid #e9ecef;
...
interface EvidenceUploadSidebarProps {
  entityName: string;
  country: string;
...
const EvidenceUploadSidebar: React.FC<EvidenceUploadSidebarProps> = ({
  entityName,
  country,
  legalRequirement,
  onClose,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Create a unique requirement ID based on entity and country
      const requirementId = `req-${entityName.replace(/\s+/g, '-').toLowerCase()}-${country.replace(/\s+/g, '-').toLowerCase()}`;
      
      await uploadEvidence(selectedFile, requirementId, description, entityName, country, legalRequirement);
      onSuccess();
      onClose();
    } catch (error) {
        ...
        <Content>
          <RequirementInfo>
            <InfoRow>
              ...
              <InfoValue>{entityName}</InfoValue>
            </InfoRow>
            <InfoRow>
            ...
            <InfoRow>
              <InfoLabel>Legal Requirement:</InfoLabel>
            </InfoRow>
...
export default EvidenceUploadSidebar;
