...
const SectionContainer = styled.div<{ isLast?: boolean }>`
  padding-bottom: 6px;
  margin-bottom: 6px;
  border-bottom: ${({ isLast }) => isLast ? 'none' : '1px solid #e5e7eb'};
`;
  ...
  return (
    <Sidebar>
      {categories.map((cat, idx) => {
        ...
        if (cat.key !== 'scope' || selectedInfoCategory === 'ED' || (Array.isArray(selectedInfoCategory) && selectedInfoCategory.includes('ED'))) {
          const isLast = categories.filter(cat2 => cat2.key !== 'scope' || selectedInfoCategory === 'ED' || (Array.isArray(selectedInfoCategory) && selectedInfoCategory.includes('ED'))).length - 1 === idx;
          return (
            <SectionContainer key={cat.key} isLast={isLast}>
